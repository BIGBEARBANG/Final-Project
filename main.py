...
with st.echo(code_location='below'):
    ...
    st.pyplot(fig_1)
    st.caption("SNRIs are getting more popular as far as GPs prescription choice goes, the older ones such as MAOIs and TCAs are being substituted for ones that cause less concern on the matter of side effects") 
    ...
    df_t=pd.concat([df_eff.dropna(axis=1), df_type_count.loc['Average amount of prescriptions']], axis = 1)
    ...
    df_ri=pd.concat([df_rub,df_tablet.loc['Average cost per tablet 2013-2019']], axis = 1).dropna(axis=1)
    ...
    df_sp=pd.concat([df_ri,[df_t.loc['Average amount of prescriptions'],df_t.loc['clinical eff']]], axis = 1).dropna(axis=1).transpose().reset_index()
